...
public class TransactionAdapter extends RecyclerView.Adapter<TransactionAdapter.TransViewHolder> {
    ...
    List<com.VipulMittal.expensemanager.accountRoom.transactionRoom.Transaction> transactions;
    ...
    public TransViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_per_item, parent, false);
        return new TransViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull TransViewHolder holder, int position) {
        com.VipulMittal.expensemanager.accountRoom.transactionRoom.Transaction transaction=transactions.get(position);
